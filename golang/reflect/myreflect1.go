...
func TestRef(s Speaker) (t reflect.Type) {
  ...
  fmt.Printf("reflect.TypeOf(%v).Elem().Type() = %v\r\n", s, t)
...
func TestRef3(s Speaker) (t reflect.Type) {
  t = reflect.TypeOf(s).Elem()
...
func main() {
  te := &Teacher{Name : "newyear", Age : 18}
  fmt.Printf("source = %v\r\n", te)
  ...
  if m, ok := reflect.New(t1).Interface().(Speaker); ok {
    fmt.Printf("reflcet.New(%v).Interface().(Speaker) = %v\r\n", t1, m)
    fmt.Printf("m.Speak() = %v\r\n", m.Speak())
    m.Set("2017")
